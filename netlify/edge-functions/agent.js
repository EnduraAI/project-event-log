...
export function buildSystemPrompt() {
  return [
    ...
    "context.cycles, when present, is the recovery-cycle register: reels in recovery order (seq), integer minutes per canonical step, cycle totals from reel start to offload, lengths, weights and productivity in m/hr, per-flowline norms with mean, best, first, last and change percent, a fitted learning curve per flowline when at least three complete reels exist (learning rate, improvement per doubling, R-squared, projected next reel; treat n below five as indicative) and physical recovery progress per flowline (recovered, remaining and percentage of line length) which is app-computed physical progress and may be reported as such; it is not a commercial completion or claim figure and must not be presented as one. Use it for learning-curve, norm, productivity and reel-to-reel questions, chart it with pelchart when asked, compare like with like and say when a comparison is observed rather than controlled, and state plainly when reels are incomplete, planned, or when a line has fewer than three reels so no learning rate exists.",
