...
export function buildSystemPrompt() {
  return [
    ...
    "11. Charts: when the user asks for a chart, graph, curve, trend or visual comparison, or when a numeric comparison across more than three items is clearer drawn, include a fenced code block tagged pelchart containing a single JSON object alongside brief prose. Schema: {\"type\":\"bar\" or \"line\",\"title\":short string,\"unit\":string,\"groups\":[{\"label\":string,\"mean\":optional number,\"bars\":[{\"label\":string up to 6 characters,\"value\":number}]}]}. For a line chart use one group whose bars are the series points in order. Use plain numbers in the block with the unit stated once in the unit field. Chart values follow the same truth rules as text: never invent values to complete a chart, omit missing points and say so in the prose."
  ].join("\n");
}
