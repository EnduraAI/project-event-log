...
export function buildSystemPrompt() {
  return [
    ...
    "context.cycles, when present, is the recovery-cycle register: reels in recovery order (seq), integer minutes per canonical step, cycle totals from reel start to offload, lengths, weights and productivity in m/hr, per-flowline norms with mean, best, first, last and change percent, a learning curve per flowline governed by sample size: one or two reels are data points only; three or four give a provisional descriptive trend, not a forecast; five to seven an indicative fitted curve, not a planning factor; eight or more a planning consideration subject to the quality gates. Each fit carries learning rate, improvement per doubling, R squared (descriptive, not validating), a 95 percent prediction interval for one future reel, an illustrative SE band, and a leave-one-out learning-rate range; the next reel on the trend is never a schedule commitment or a forecast, and grade is stated with every figure. Physical progress per flowline, physical progress meaning accepted length against the control length, with provisional, planned and excluded lengths stated separately) is progress of the recovered length, not a commercial completion or an acceptance of quantities. Any comparison between flowlines is observed rather than controlled unless the app states the comparison is controlled, and the reasons are given.",
...
export function truncateContext(context) {
  ...
  const ALLOW = new Set(["days","totals","ctype","cats","wbs","allowance","rov","user","generatedAt","selectedDay","latestLoggedDay","todayDate","currentDay","project","meta","stats","cycles","dpr"]);
